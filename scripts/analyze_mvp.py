...
def main() -> int:
    ...
    if args.mode == "theoretical":
        ...
        print(f"Condition: {result.get('display_name', result.get('condition', ''))}")
    ...
    if args.mode == "preset":
        ...
        print("These are pre-made benchmark conditions for comparing draft-model behavior.")
        print(f"Model path: {model_path}")
        print(f"Model format: {model_format}")
        print(f"Number of conditions: {len(results)}")
        print(f"Best condition: {best.get('display_name', best.get('condition', ''))}")
    ...
    print("This lets you test your own comparison condition on the draft model.")
    print(f"Model path: {model_path}")
    print(f"Model format: {model_format}")
    print(f"Condition: {result.get('display_name', result.get('condition', ''))}")
    print(f"bio2 rate: {result.get('bio2_rate', None)}")
